...
int read_fd(int fd, int *recvfd, int *port, struct sockaddr_un *name);
...
int main(int argc, char **argv) {
    ...
    sprintf(server_name.sun_path, "Xclient.unix");    // On Linux, sun_path is 108 bytes in size.
    ...
    sprintf(client_name.sun_path, "Xserver.unix");    	// On Linux, sun_path is 108 bytes in size.
    client_name.sun_path[0] = '\0';                 	// Abstract linux socket.

	int rsock; int port;
	if (read_fd(commfd, &rsock, &port, &client_name) < 0) {
